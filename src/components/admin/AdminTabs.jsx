export default function AdminTabs({ activeTab, onTabChange, counts }) {
  const tabs = [
    ...
    { id: 'pdfs', label: 'PDF Purchases', icon: '📄', badge: counts.pdfs }
  ];
    ...
    <div className="adm-tabs bg-white border-b border-gray-200 flex px-7 gap-1 overflow-x-auto shadow-sm">
      {tabs.map(tab => (
        <button
          ...
          className={`adm-tab flex items-center gap-2 px-4 py-3 text-sm font-bold whitespace-nowrap border-b-2 transition ${activeTab === tab.id
        ...
        >
          ...
          <span>{tab.label}</span>
          <span
            className={`tab-badge px-2 py-0.5 rounded-full text-[10px] font-black ${activeTab === tab.id
