                ...
                <tr key={m.id} className="hover:bg-indigo-50/30 border-b border-gray-100 transition">
                  ...
                  <td className="px-3 py-2 text-xs font-bold text-gray-900">
                    <div className="flex items-center gap-2">
                      <MentorAvatar name={m.full_name} index={i} />
                  ...
                  <td className="px-3 py-2 text-xs font-bold text-gray-700 whitespace-nowrap">
                    <div className="flex items-center gap-1">
                      <span className="inline-block w-[88px]">{m.phone || '—'}</span>
...
function MentorAvatar({ name, index }) {
  const initials = (name || '?')
    .split(' ')
    .map(w => w[0])
    .join('')
  ...
  return (
    <div className={`w-7 h-7 rounded-full bg-gradient-to-br ${color} text-white flex items-center justify-center text-[10px] font-black flex-shrink-0`}>
      {initials}
    </div>
  );
}
