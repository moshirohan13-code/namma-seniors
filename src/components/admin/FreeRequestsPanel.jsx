...
export default function FreeRequestsPanel({ freeRequests, onRefresh, showToast }) {
  ...
  async function deleteRequest(id) {
    if (!window.confirm('Delete this free session request permanently? This cannot be undone.')) return;
    try {
        ...
        <table className="w-full border-collapse">
          <thead>
            <tr className="bg-indigo-50/50">
              ...
              <th className="px-3 py-2 text-left text-[10px] font-black text-gray-400 uppercase tracking-wider border-b border-gray-100">Needs Help With</th>
                    ...
                    <td className="px-3 py-2 text-xs text-gray-700 max-w-[180px]">
                      {b.requirement_message ? (
                        <div className="flex items-start gap-1">
                          <span className="line-clamp-2">{b.requirement_message}</span>
                          <button
                            onClick={() => alert(b.requirement_message)}
